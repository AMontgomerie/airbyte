...
def SnowflakeConnect(config):
    conn = snowflake.connector.connect(
        user=config["username"], password=config["password"], account=config["account"]
    )
    cursor = conn.cursor()
...
def read(config, catalog):
    ...
    try:
        ...
        for i in range(0, len(data)):
            ...
            output_message = {
                "type": "RECORD",
                "record": {"stream": f"{table}", "data": row_json},
            }
...
def check(config):
    ...
    try:
        ...
        output_message = {
            "status": "FAILED",
            "message": "Input configuration is incorrect. Please verify the input in config.json.",
        }
        print(json.dumps(output_message))
...
def run(args):
    ...
    check_parser = subparsers.add_parser(
        "check", help="checks the config used to connect", parents=[parent_parser]
    )
